...
import { appUrl, gatewayConfigured, gatewayCurrencyInfo, initiateHostedPayment, storeRedirectPage, toGatewayAmountCents } from "@/lib/payments/powertranz"
...
// bought. The charge covers base + currently-enabled add-ons
// (bundle pricing auto-applied), and the customer may settle
// several months in one go — a full year earns 2 months free.
//============================================================
...
export async function startTenantCheckout(tenantId: string, monthsRaw: number = 1): Promise<{ url: string }> {
    ...
    const gateway = await initiateHostedPayment({
        amountCents: gatewayAmountCents,
