...
export default function ViewProject({ projectFromServer }: { projectFromServer: project }) {
    ...
    async function updateProjectsToTemplate(choiceObj: updateProjectsToTemplateFunctionType): Promise<boolean> {
            ...
            try {
                ...
                syncDebounce.current = setTimeout(async () => {
                    if (seenUpdatedProjectsToTemplates === null) {
                        console.log(`$not seeing update that just happened on local set state to sync with server`);
                        return
                    }
