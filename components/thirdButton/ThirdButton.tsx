...
export default function ThirdButton({ link, text, isHovering }: { link: string, text: string, isHovering?: boolean }) {
    return (
        <Link href={link} style={{ display: "grid" }}>
            <button className={`button3 ${isHovering && "hovering"}`} style={{ padding: "0 0 0 var(--spacingR)" }}>
                <p>{text}</p>

                <div style={{ height: '100%', display: "grid", alignItems: "center", justifyItems: "center", padding: "var(--spacingR) 3rem var(--spacingR) 3rem", transform: "skew(-29deg)", translate: "1rem" }}>
