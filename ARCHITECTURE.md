...
1. **Data shapes** (`lib/sites/content.ts`): one Zod schema per category (navbar with menus/sub-menus, hero, text, services, gallery, testimonials, hours, announcement, contact, booking, products, footer) in a discriminated union on `data.category`. `defaultComponentData()` seeds new instances. Site-wide meta (business profile + SEO) lives on `tenants.content` — components read it via `ctx`, so a phone-number change applies everywhere.
...
`lib/sites/addons.ts` — $5 base + $5 each: **booking** (availability rules + slot math + advisory-locked conflict check; bookable services live on the booking component's own data), **notifications** (owner emails for bookings/messages, customer announcement blasts via `handleNotifications.tsx` + `tenantAnnouncements` history, WhatsApp deep links), **inventory** (`tenantProducts`/`tenantSales`, integer cents + basis-point tax, advisory-locked stock decrement, month-to-date summary, public shop section with wa.me ordering), **custom-domain**. `monthlyTotal` tolerates retired ids on old rows.
